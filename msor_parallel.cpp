...
vector<double> solveLU(vector<vector<double> > A, vector<vector<double> > D, vector<vector<double> > L, vector<vector<double> > U, vector<vector<double> > omega, vector<double> q, vector<double> x, double alpha)
{
  ...
  vector<vector<double> > DplusU(n, vector<double> (n, 0));
  ...
  // generate (1-alpha)D + alpha*U
#pragma omp parallel shared(D, U, alpha) private(i, j) 
  { 
  #pragma omp for
    for(i=0; i<n; i++){
      for(j=0; j<n; j++){
        DplusU[i][j] = (1 - alpha)*D[i][j] + alpha*U[i][j];

      }
    }
  }
  vector<double> g = matrixMultiplyVector(DplusU, x);
